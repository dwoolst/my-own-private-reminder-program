...
class KirbyBase:
    ...
    def _sendSocket(self, command):
        ...
        # First, send command length.
        dbSock.send('%16s' % len(command))

        # Next, send the method call to the server.
        dbSock.sendall(command)

        # Receive the return value of the method.
        recv_length = int(dbSock.recv(1024))

        # Keep receiving data until length of data received equals
        # recv_length.
        data = cStringIO.StringIO()

        while len(data.getvalue()) < recv_length:
            data.write(dbSock.recv(recv_length))

        # Convert pickled binary data back into it's original format
        # (usually a list).
        data = cPickle.loads(data.getvalue())
